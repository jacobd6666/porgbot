...
# -----------------------------------------------------------------------------
@bot.command
@lightbulb.add_checks(lightbulb.has_roles(MASTER_CODEBREAKER)
                      )  # mode=all or mode=any
...
async def update_all(ctx):
	...
	equipment_locations_dict = process_equipment_locations_data(
	    gear_locations_response)
...
@lightbulb.option(
    name='charname',
    description='Name of character to update (existing nickname is acceptable)'
)
@lightbulb.command(name='addnickname',
                   description='Add nickname to nickname list')
@lightbulb.implements(lightbulb.SlashCommand)
...
@lightbulb.option(
    name='tiers',
    description=
    'Tiers to be included. Either ALL, a single number, or a range of numbers eg. 1-3'
)
@lightbulb.option(name='charname', description='Name of character')
@lightbulb.command(
    name='components',
    description=
    'Get the components for the provided tiers for the provided character')
@lightbulb.implements(lightbulb.SlashCommand)
async def get_components(ctx):
	...
	if len(values) > 25:
		print(
		    'More than 25 fields - the last {} will be missing from the response'
		    .format(values - 25))
	...
	embedVar = hikari.Embed(
	    ...
	    description=
	    "**Requested tier(s)** \u27A4 {}\n**Requested char** \u27A4 {}".format(
	        ctx.options['tiers'], charObject['name']),
	    color=colour)
	embedVar.set_thumbnail(charObject['image'])
	embedVar.set_footer(text="Requested by {}".format(ctx.author.username),
	...
	if len(sorted_dict.items()) > 0:
		for i in values:
			...
			for key, value in sorted_dict.items():
				if value == i:
					string += '{}\n'.format(key)
			embedVar.add_field(name="{}x".format(i),
			                   value=string,
			                   inline=False)
	else:
		embedVar.add_field(
		    name="No components found",
		    value="Please check the character name and tier(s) provided",
		    inline=False)
...
# -----------------------------------------------------------------------------
@bot.command
@lightbulb.option(name='gearname',
                  description='Name of the gear to get information for')
@lightbulb.command(name='gearinfo',
                   description='Get information about a piece of gear')
@lightbulb.implements(lightbulb.SlashCommand)
async def get_gear_info(ctx):
	...
	# start to build the response embed
	embedVar = hikari.Embed(title="{}".format(gearObject['name']),
	...
	# if the gear has no farmable nodes, add a message to the embed
		...
		embedVar.add_field(
		    ...
		    value=
		    'This gear is not found in any farmable nodes.\n It is likely that you will need to farm the components separately',
		    inline=False)
...
# -----------------------------------------------------------------------------
@bot.command
@lightbulb.option(name='targetrelic',
                  description='Number of target relic',
                  type=int)
@lightbulb.option(name='nextrelic',
                  description='Number of first relic to get components for',
                  type=int)
@lightbulb.command(
    name='reliccomponents',
    description='Get the required components for a range of relics')
@lightbulb.implements(lightbulb.SlashCommand)
async def get_relic_mats(ctx):
	if ctx.options['nextrelic'] > ctx.options['targetrelic']:
		await ctx.respond(
		    'The first relic number must be less than the target relic number')
		return
	relic_mats = get_relic_components(ctx.options['nextrelic'],
	                                  ctx.options['targetrelic'])

	embedVar = hikari.Embed(
	    title="Relic components",
	    description="**Requested relics** \u27A4 {}-{}".format(
	        ctx.options['nextrelic'], ctx.options['targetrelic']),
	    color=0xffffff)
	embedVar.set_footer(text="Requested by {}".format(ctx.author.username),
	                    icon=ctx.author.avatar_url)
		...
		if k == 'Credits':
			...
			for key, value in v.items():
				signal_data_string += '`{1}x{2}{0}`\n'.format(
				    key, str(value), space * (5 - len(str(value))))
